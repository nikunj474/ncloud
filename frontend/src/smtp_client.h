...
// Message-building helpers — used by both relay and direct paths.
...
inline std::string mime_boundary_for() {
    auto now = std::chrono::system_clock::now().time_since_epoch();
    auto ns = std::chrono::duration_cast<std::chrono::nanoseconds>(now).count();
    static std::mt19937 rng(static_cast<unsigned>(ns ^ 0x5A17u));
    std::uniform_int_distribution<unsigned> dist(0, 0xFFFFFF);
    std::ostringstream oss;
    oss << "----=_PennCloud_" << ns << "_"
        << std::hex << std::setw(6) << std::setfill('0') << dist(rng);
    return oss.str();
}

inline std::string sanitize_content_type(std::string mime) {
    mime = sanitize_header_text(mime);
    if (mime.empty() || mime.find('/') == std::string::npos) return "application/octet-stream";
...
inline std::string base64_encode_wrapped(const std::string& data) {
    ...
    while (i + 2 < data.size()) {
        ...
        emit(table[a >> 2]);
        emit(table[((a & 0x03) << 4) | (b >> 4)]);
        emit(table[((b & 0x0F) << 2) | (c >> 6)]);
        emit(table[c & 0x3F]);
    }
...
inline std::string build_message(const std::string& from_addr,
                                 ...
                                 const std::vector<SMTPAttachment>& attachments = {}) {
    std::ostringstream oss;
    oss << "From: ";
    ...
    std::string boundary = mime_boundary_for();
    oss << "Content-Type: multipart/mixed; boundary=" << quote_header_phrase(boundary) << "\r\n";
    oss << "\r\n";
    oss << "This is a MIME message.\r\n";
    oss << "--" << boundary << "\r\n";
    oss << "Content-Type: text/plain; charset=UTF-8\r\n";
    oss << "Content-Transfer-Encoding: 8bit\r\n";
    oss << "\r\n";
    ...
    for (const auto& attachment : attachments) {
        std::string filename = sanitize_header_text(attachment.filename);
        if (filename.empty()) filename = "attachment";
        std::string content_type = sanitize_content_type(attachment.content_type);
        oss << "--" << boundary << "\r\n";
        oss << "Content-Type: " << content_type << "; name=" << quote_header_phrase(filename) << "\r\n";
        oss << "Content-Transfer-Encoding: base64\r\n";
        oss << "Content-Disposition: attachment; filename=" << quote_header_phrase(filename) << "\r\n";
        oss << "\r\n";
        oss << base64_encode_wrapped(attachment.data) << "\r\n";
    }
...
inline SMTPExternalResult smtp_send_via_curl_cli(const std::string& from_addr,
    ...
    std::string message = build_message(from, to_addr, subject, body, reply_to, display_from, from_addr, attachments);
...
#ifdef USE_CURL_RELAY
// libcurl relay structs and functions — only compiled when libcurl is present.
...
inline CURLcode ensure_curl_global_init() {
    ...
    std::call_once(once, [] {
        init_rc = curl_global_init(CURL_GLOBAL_DEFAULT);
    });
    return init_rc;
}
...
inline SMTPExternalResult smtp_send_via_relay(const std::string& from_addr,
    ...
    std::string url = std::string("smtp://") + host + ":" + port;
    UploadStatus upload;
    upload.payload = build_message(from, to_addr, subject, body, reply_to, display_from, from_addr, attachments);
    ...
    if (rc == CURLE_OK) {
        ...
        std::string detail = curl_err[0] ? curl_err : curl_easy_strerror(rc);
        res.error = "relay send failed: " + detail;
    }
...
inline SMTPExternalResult smtp_send_via_relay(const std::string& from_addr,
                                              ...
                                              const std::string& body,
                                              const std::vector<SMTPAttachment>& attachments = {}) {
    return smtp_send_via_curl_cli(from_addr, to_addr, subject, body, attachments);
}
...
inline bool try_send_once(const std::string& mx_host,
    ...
    std::string msg = build_message(from_addr, to_addr, subject, body, "", "", "", attachments);
    if (!write_all(fd, msg + "\r\n.\r\n")) {
...
inline SMTPExternalResult smtp_send_direct(const std::string& from_addr,
    ...
    for (const auto& rec : mx) {
        ...
        if (try_send_once(rec.host, from_addr, to_addr, subject, body, attachments, last_err, temp_fail)) {
            res.ok = true;
            return res;
        }
...
inline SMTPExternalResult smtp_send_external(const std::string& from_addr,
    ...
    if (smtp_client_detail::domain_matches_direct_list(
        ...
        SMTPExternalResult direct = smtp_client_detail::smtp_send_direct(from_addr, to_addr, subject, body, attachments);
        ...
        SMTPExternalResult relay = smtp_client_detail::smtp_send_via_relay(from_addr, to_addr, subject, body, attachments);
        if (!relay.ok) {
    ...
    return smtp_client_detail::smtp_send_direct(from_addr, to_addr, subject, body, attachments);
}
