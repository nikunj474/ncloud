...
static HttpResponse api_unauthorized() {
    HttpResponse r = HttpResponse::json(
        R"({"ok":false,"authenticated":false,"error":"unauthorized"})");
    r.status_code = 401;
...
// so the admin panel works regardless of which port set the cluster uses.
...
static AdminClusterSpec detect_active_cluster_admin(const FEServer::Config& cfg) {
    ...
    // If the frontend was started with an explicit coord-port, and that
    // coordinator is actually up, prefer a known demo topology when ports
    // match one. Otherwise derive from cfg so non-standard KV ports (e.g.
    // 5050) are handled correctly.
    ...
    // Fallback: probe well-known coordinator ports and return matching preset.
    ...
    // Last resort: derive from cfg even without a live coordinator.
...
// Spawn a plain 404 stub on a killed frontend port. This keeps the old URL
// reachable for demo inspection while still making health probes mark it down.
static void start_fe_404_stub(int killed_port, const std::string& fe_bin) {
    ...
    std::string cmd =
        ...
        " --page-not-found-stub" +
        " > /dev/null 2>&1 &";
    admin_log("starting 404 stub on killed frontend port " + std::to_string(killed_port));
...
static bool perform_admin_control(const FEServer::Config& cfg, const std::string& kind, const std::string& action,
    ...
    if (kind == "frontend") {
        ...
        if (action == "kill") {
            ...
            // Non-self kill: kill immediately, then start a 404 stub on the freed port.
            std::thread([target, fe_port, fe_bin] {
                kill_ports_fast_admin("node " + target, {fe_port});
                std::this_thread::sleep_for(std::chrono::milliseconds(100));
                start_fe_404_stub(fe_port, fe_bin);
            }).detach();
            message = "kill scheduled for " + target;
            return true;
        }
        if (action == "restart") {
            ...
            if (fe_port == cfg.port) {
                ...
                // sleep briefly to flush the HTTP response, then kill instantly and
                // wait so the kernel fully releases the port before the new
                // process tries to bind it.
                std::string cmd = "( sleep 0.2; " + kill_port_cmd_admin(fe_port) +
                                  "; sleep 0.2; " + launch + " ) >/dev/null 2>&1 &";
...
void FEServer::handle_connection(int fd) {
    // 600 s idle timeout — long enough for a slow 128 MB upload from the browser
...
bool FEServer::handle_one_request(int fd) {
    ...
    if (req.path == "/events" && req.method == "GET") {
        ...
        if (user.empty()) {
            ...
            struct SseSlotGuard {
                FEServer* server;
                ~SseSlotGuard() { server->release_sse_slot(); }
            } guard{this};
...
// Route dispatch
// ---------------------------------------------------------------------------
static std::string sid_from_req(const HttpRequest& req) {
    std::string sid = req.header("x-session-id");
    if (sid.empty()) sid = req.param("sid");
    if (sid.empty()) sid = req.cookie("sid");
    return sid;
}

std::string FEServer::get_user(const HttpRequest& req) {
    return sessions_->validate(sid_from_req(req));
}
...
HttpResponse FEServer::dispatch(const HttpRequest& req) {
    ...
    if (path == "/api/upload" && method == "POST") {
        if (user.empty()) return api_unauthorized();
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    :root {
      ...
      --success:   #1A6B3A;
      --danger:    #C53030;
      --shadow:    0 12px 34px rgba(1,31,91,0.10);
      --sidebar-w: 232px;
      --topbar-h:  64px;
    }
    ...
    .penn-logo { width: 38px; height: 44px; flex: 0 0 auto; display: block; }
    .penn-logo.small { width: 30px; height: 36px; }
    ...
    .brand-subtitle { margin-top: 4px; font-size: 12px; font-weight: 500; color: rgba(255,255,255,0.72); }
    ...
    #login-page {
      ...
      background:
        linear-gradient(115deg, rgba(153,0,0,0.32) 0 24%, transparent 24% 100%),
        var(--penn-blue);
    }
    .login-card {
      background: var(--surface); border-radius: 14px;
      padding: 34px 34px 32px; width: 390px;
      border: 1px solid rgba(255,255,255,0.16);
      box-shadow: 0 24px 70px rgba(0,0,0,0.32);
    }
    .login-brand { margin-bottom: 24px; color: var(--penn-blue); }
    .login-brand .brand-name { font-size: 24px; }
    .login-brand .brand-subtitle { color: var(--muted); font-size: 13px; }
    ...
    .form-group input {
      ...
      border-radius: 8px; font-size: 14px; outline: none; transition: border .15s, box-shadow .15s;
      background: #fff;
    }
    .form-group input:focus { border-color: var(--accent); box-shadow: 0 0 0 3px rgba(0,102,204,0.12); }
    .btn {
      width: 100%; padding: 11px; border: none; border-radius: 8px;
      font-size: 14px; font-weight: 700; cursor: pointer; transition: transform .15s, box-shadow .15s, background .15s, opacity .15s;
    }
    .btn-primary { background: var(--penn-blue); color: white; box-shadow: 0 9px 20px rgba(1,31,91,0.22); }
    ...
    .topbar {
      ...
      box-shadow: 0 5px 22px rgba(1,31,91,0.18);
      position: sticky; top: 0; z-index: 20;
    }
    .topbar .user-info { font-size: 13px; display: flex;
                         align-items: center; gap: 12px; }
    #topbar-user { max-width: 220px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;
                   background: rgba(255,255,255,0.12); border: 1px solid rgba(255,255,255,0.18);
                   padding: 6px 10px; border-radius: 999px; }
    .topbar .logout-btn { background: rgba(255,255,255,0.15); border: 1px solid rgba(255,255,255,0.24);
    ...
    .nav-item {
      ...
      font-size: 14px; color: #4A5568; transition: background .12s, color .12s, box-shadow .12s;
      border: none; background: none; width: 100%; text-align: left;
    }
    .nav-item:hover { background: var(--surface-2); color: var(--text); }
    .nav-item.active { background: #EAF2FB; color: var(--penn-blue);
                       font-weight: 700; box-shadow: inset 3px 0 0 var(--penn-red); }
    .nav-icon { width: 18px; text-align: center; font-size: 16px; }
    .content { flex: 1; padding: 28px 32px; overflow-y: auto; background:
    ...
    .compose-btn {
      ...
      font-size: 13px; font-weight: 700; box-shadow: 0 8px 18px rgba(1,31,91,0.16);
    }
    .compose-btn:hover { background: #01266E; }
    ...
    .action-btn {
      ...
      color: var(--text); transition: background .12s, border-color .12s, transform .12s;
    }
    .action-btn:hover { background: var(--surface-2); border-color: #A0AEC0; }
    .action-btn.danger { color: var(--danger); border-color: var(--danger); }

    /* ---- Compose ---- */
    .compose-form { background: var(--surface); border-radius: 10px;
                    border: 1px solid var(--border); padding: 28px; max-width: 760px; box-shadow: var(--shadow); }
    ...
    .drive-top {
      ...
      gap: 20px; align-items: center; box-shadow: var(--shadow);
    }
    .drive-eyebrow { color: var(--penn-red); font-size: 12px; font-weight: 800; text-transform: uppercase; }
    ...
    .drive-quota { background: var(--surface-2); border: 1px solid var(--border); border-radius: 9px; padding: 12px; }
    ...
    .drive-table-header {
      padding: 10px 18px; background: var(--surface-2); color: #4A5568;
      font-size: 12px; font-weight: 800; text-transform: uppercase; border-bottom: 1px solid var(--border);
    }
    .drive-row { padding: 12px 18px; border-bottom: 1px solid var(--border); min-height: 64px; }
    ...
    .drive-icon {
      ...
      flex: 0 0 auto; font-size: 12px; font-weight: 800; color: var(--penn-blue); background: #EAF2FB;
      border: 1px solid #D3E2F2;
    }
    .drive-icon.folder { background: #FFF5E5; color: #8A4B00; border-color: #F3D4A3; }
    .drive-icon.file { background: #F0F7F4; color: var(--success); border-color: #CFE7D8; }
    ...
    .drive-row-actions button {
      ...
      border: 1px solid var(--border-2); cursor: pointer; background: var(--surface);
      color: #2D3748; font-weight: 600;
    }
    .drive-row-actions button:hover { background: var(--surface-2); }
    .drive-row-actions .danger { color: var(--danger); border-color: rgba(197,48,48,0.45); }
    .drive-empty {
      padding: 46px 24px; text-align: center; color: var(--muted);
      display: flex; flex-direction: column; align-items: center; gap: 12px;
    }
    .drive-empty-icon {
      width: 56px; height: 56px; border-radius: 14px; background: #EAF2FB; color: var(--penn-blue);
    ...
    @media (max-width: 860px) {
      .topbar { height: auto; min-height: var(--topbar-h); padding: 12px 16px; gap: 12px; }
      .brand-subtitle { display: none; }
      .main-layout { flex-direction: column; }
      .sidebar {
        width: 100%; border-right: none; border-bottom: 1px solid var(--border);
        padding: 8px; flex-direction: row; overflow-x: auto;
      }
      .nav-item { min-width: 118px; justify-content: center; }
      .nav-item.active { box-shadow: inset 0 -3px 0 var(--penn-red); }
    ...
    @media (max-width: 520px) {
      ...
      .topbar .user-info { width: 100%; justify-content: space-between; }
      #topbar-user { max-width: 60vw; }
    ...
    .upload-progress {
      position: fixed; right: 24px; bottom: 76px; width: min(360px, calc(100vw - 32px));
      background: var(--surface); border: 1px solid var(--border); border-radius: 10px;
      box-shadow: 0 16px 42px rgba(1,31,91,0.18); padding: 14px; z-index: 998;
      display: none;
    }
    .upload-progress.show { display: block; }
    .upload-progress-top { display: flex; align-items: center; justify-content: space-between; gap: 12px; margin-bottom: 8px; }
    .upload-progress-name { min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; font-size: 13px; font-weight: 800; color: var(--penn-blue); }
    .upload-progress-percent { font-size: 12px; font-weight: 800; color: #4A5568; }
    .upload-progress-track { height: 8px; background: #E2E8F0; border-radius: 999px; overflow: hidden; }
    .upload-progress-fill { height: 100%; width: 0%; background: var(--accent); border-radius: inherit; transition: width .12s linear; }
    .upload-progress-meta { display: flex; justify-content: space-between; gap: 10px; margin-top: 8px; color: var(--muted); font-size: 12px; }
    .upload-progress-status { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
    .upload-progress.done .upload-progress-fill { background: var(--success); }
    .upload-progress.error .upload-progress-fill { background: var(--danger); }
  </style>
</head>
<body>
...
<div id="login-page">
  <div class="login-card">
    <div class="login-brand logo-lockup">
      <svg class="penn-logo" viewBox="0 0 48 56" aria-hidden="true" focusable="false">
        <path d="M24 2 44 10v17c0 13-8.3 22.5-20 27C12.3 49.5 4 40 4 27V10L24 2z" fill="#011F5B"/>
        <path d="M8 11.5h32v12H8z" fill="#990000"/>
        <path d="M12 15h24v2.8H12zM12 20h24v2.8H12z" fill="#fff" opacity=".92"/>
...
<div id="app">
  <div class="topbar">
    <div class="brand logo-lockup">
      <svg class="penn-logo small" viewBox="0 0 48 56" aria-hidden="true" focusable="false">
        <path d="M24 2 44 10v17c0 13-8.3 22.5-20 27C12.3 49.5 4 40 4 27V10L24 2z" fill="#fff"/>
        <path d="M8 11.5h32v12H8z" fill="#990000"/>
        <path d="M12 15h24v2.8H12zM12 20h24v2.8H12z" fill="#fff" opacity=".9"/>
...
<div class="upload-progress" id="upload-progress" aria-live="polite">
  <div class="upload-progress-top">
    <div class="upload-progress-name" id="upload-progress-name">Uploading</div>
  ...
  <div class="upload-progress-meta">
    <span class="upload-progress-status" id="upload-progress-status">Starting...</span>
    <span id="upload-progress-rate">0B/s</span>
  </div>
</div>
...
function apiFetch(url, opts) {
  ...
  const sid = sessionStorage.getItem(_sidKey) || '';
  if (sid) opts.headers = Object.assign({'X-Session-Id': sid}, opts.headers || {});
  return fetch(url, opts);
}

function cachedDisplayUser() {
  try {
...
function rememberDisplayUser(user) {
  const clean = String(user || '').trim();
  if (!clean) return;
  try {
    localStorage.setItem(lastUserKey, clean);
  } catch (_) {}
}

function clearDisplayUser() {
  try {
    localStorage.removeItem(lastUserKey);
  } catch (_) {}
}

function updateTopbarUser(fallback = 'Recovering...') {
  const el = document.getElementById('topbar-user');
  if (!el) return;
  el.textContent = currentUser || cachedDisplayUser() || fallback;
}
...
function renderStorageDegraded(message = 'Storage is temporarily unavailable. Waiting for replica recovery...') {
  document.getElementById('login-page').style.display = 'none';
  document.getElementById('app').style.display = 'flex';
  updateTopbarUser();
  const content = document.getElementById('content');
  if (content) {
    content.innerHTML = `<div class="spinner">${escHtml(message)}</div>`;
  }
}
...
function redirectToPeerApp() {
  ...
  let query = queryWithDownFrontend(currentId);
  const _rsid = sessionStorage.getItem(_sidKey) || '';
  if (_rsid) query += (query.includes('?') ? '&' : '?') + '_sid=' + encodeURIComponent(_rsid);
...
async function backendStorageAvailable() {
  ...
  try {
    const r = await apiFetch('/api/admin/status', { cache: 'no-store', signal: controller.signal });
...
function formatBytes(n) {
  ...
  const compact = (value, digits = 1) => value.toFixed(digits).replace(/\.0+$|(\.\d*[1-9])0+$/, '$1');
  if (num < 1024) return `${Math.round(num)}B`;
  if (num < 1024 * 1024) return `${compact(num / 1024)}KB`;
  if (num < 1024 * 1024 * 1024) return `${compact(num / (1024 * 1024))}MB`;
  return `${compact(num / (1024 * 1024 * 1024), 2)}GB`;
}

function formatQuotaUsedBytes(n) {
  const num = Number(n || 0);
  if (num < 10 * 1024 * 1024 * 1024) {
...
function bindEnterSubmit(root, selector, action, options = {}) {
  const scope = typeof root === 'string' ? document.querySelector(root) : root;
  if (!scope) return;
  scope.querySelectorAll(selector).forEach(el => {
    if (el.dataset.enterSubmitBound === '1') return;
    el.dataset.enterSubmitBound = '1';
    el.addEventListener('keydown', e => {
      if (!isPlainEnter(e)) return;
      const tag = (e.target && e.target.tagName ? e.target.tagName.toLowerCase() : '');
      if (tag === 'textarea' && !options.allowTextarea) return;
      e.preventDefault();
      e.stopPropagation();
      action(e);
    });
...
function clickPrimaryButton(selector) {
  const btn = document.querySelector(selector);
  if (btn && !btn.disabled) btn.click();
}

function bindLoginEnter() {
  bindEnterSubmit(document.getElementById('login-page'), '#username-in, #password-in', () => {
    clickPrimaryButton('#login-btn');
  });
}

function bindComposeEnter() {
  const form = document.querySelector('.compose-form');
  bindEnterSubmit(form, '#to-in, #subject-in', () => sendEmail());
}

function bindChatEnter() {
  bindEnterSubmit(document.getElementById('chat-input') ? document : null, '#chat-input', () => sendChatMessage(), {
    allowTextarea: true
  });
}
...
function bindSettingsEnter() {
  const content = document.getElementById('content');
  bindEnterSubmit(content, '#old-pw, #new-pw, #new-pw2', () => changePassword());
  bindEnterSubmit(content, '#quota-mb', () => updateQuota());
  bindEnterSubmit(content, '#contact-name, #contact-email', () => addContact());
}
...
async function doLogin() {
  ...
  const data = await r.json();
  if (data.ok) {
    if (data.sid) sessionStorage.setItem(_sidKey, data.sid);
    currentUser = u;
    rememberDisplayUser(u);
    contactsCache = [];
    quotaCache = null;
    await showAppForCurrentRoute();
  } else {
...
async function doLogout() {
  cleanupClientSession();
  sessionStorage.removeItem(_sidKey);
  try {
...
function isProtectedAppPath(path) {
  return ['/inbox', '/compose', '/drive', '/chat', '/email', '/settings'].includes(path || '');
}

async function checkInitialAppRouteHealth() {
  if (!isProtectedAppPath(window.location.pathname || '/')) return;
  const available = await backendStorageAvailable();
  if (available === false) renderPageNotFound();
}

function normalizeDrivePathClient(path) {
...
function routeFromLocation() {
  const path = window.location.pathname || '/';
  if (path === '/inbox') return {view: 'inbox', params: {folder: 'inbox'}};
  if (path === '/compose') return {view: 'compose', params: {}};
  if (path === '/drive') {
    const qs = new URLSearchParams(window.location.search || '');
    return {view: 'drive', params: {path: normalizeDrivePathClient(qs.get('path') || '/')}};
  }
  if (path === '/chat') return {view: 'chat', params: {}};
  if (path === '/email') return {view: 'inbox', params: {folder: 'inbox'}};
...
function routeUrl(view, params = {}) {
  ...
  const down = current.get(frontendDownParam);
  if (down) qs.set(frontendDownParam, down);
  if (view === 'drive') {
    const drivePath = normalizeDrivePathClient(params.path || '/');
    if (drivePath !== '/') qs.set('path', drivePath);
  }
  const query = qs.toString();
  return '/' + view + (query ? '?' + query : '');
}

async function showAppForCurrentRoute() {
  const route = routeFromLocation();
  if (route) {
    await showApp(route.view, route.params, true);
    return;
  }
  await showApp();
}
...
async function fetchSessionUser() {
  ...
  const timeout = setTimeout(() => controller.abort(), 700);
  try {
    const r = await apiFetch('/api/session', { cache: 'no-store', signal: controller.signal });
    if (r.status === 503) return {unavailable: true};
    if (!r.ok) return {authenticated: false};
    const data = await r.json();
    return {
      authenticated: !!(data && data.ok && data.authenticated),
...
async function bootstrapInitialRoute() {
  ...
  const available = await backendStorageAvailable();
  if (available === false) { renderPageNotFound(); return; }
  if (available !== true) return;

  const session = await fetchSessionUser();
  if (session.unavailable) {
    renderStorageDegraded();
    return;
  }
  ...
  rememberDisplayUser(currentUser);
  await showApp(route.view, route.params, true);
}

async function showApp(initialView = 'inbox', initialParams = {folder: 'inbox'}, replaceRoute = false) {
  document.getElementById('login-page').style.display = 'none';
  document.getElementById('app').style.display = 'flex';
  updateTopbarUser();
  authActive = true;
  ...
  navigate(initialView, initialParams, replaceRoute);
}
...
// ---- Navigation ------------------------------------------------------------
function navigate(view, params = {}, replaceRoute = false) {
  currentView = view;
  ...
  if (replaceRoute) history.replaceState({view, params}, '', routeUrl(view, params));
  else history.pushState({view, params}, '', routeUrl(view, params));
...
window.addEventListener('popstate', e => {
  if (e.state) navigate(e.state.view, e.state.params || {}, true);
});

// Expose frequently used handlers explicitly for reliability with dynamic views.
window.navigate = navigate;
window.openDrivePath = openDrivePath;
...
async function renderInbox(folder = 'inbox', _retry = 0) {
  ...
  const results = await Promise.all(
    folders.map(async f => {
      try {
        const r = await apiFetch('/api/inbox?folder=' + encodeURIComponent(f));
        const data = await r.json();
        return data;
      } catch (_) {
...
async function renderEmail(uid, folder = 'inbox') {
  currentMailFolder = folder;
  const r = await apiFetch(`/api/email/${encodeURIComponent(uid)}?folder=${encodeURIComponent(folder)}`);
  const data = await r.json();
  const content = document.getElementById('content');
...
async function deleteEmail(uid, folder = currentMailFolder || 'inbox') {
  try {
    const r = await apiFetch('/api/delete-email', {
      method: 'POST',
      headers: {'Content-Type': 'application/x-www-form-urlencoded'},
...
async function restoreEmail(uid) {
  try {
    const r = await apiFetch('/api/restore-email', {
      method: 'POST',
      headers: {'Content-Type': 'application/x-www-form-urlencoded'},
...
async function uploadMailAttachment(file) {
  ...
  try {
    const r = await apiFetch('/api/mail/upload-attachment', { method: 'POST', body: fd });
    const data = await r.json();
    if (!data.ok) {
...
async function addCurrentRecipientToContacts() {
  ...
  if (!name) return;
  const r = await apiFetch('/api/contacts/add', {
    method: 'POST',
    headers: {'Content-Type': 'application/x-www-form-urlencoded'},
...
async function loadDmPeers() {
  try {
    const r = await apiFetch('/api/chat/dms', { cache: 'no-store' });
    const data = await r.json();
...
async function syncDmSidebarState() {
  try {
    const peerResp = await apiFetch('/api/chat/dms', { cache: 'no-store' });
    ...
    const results = await Promise.all(peers.map(async peer => {
      try {
        const r = await apiFetch('/api/chat/dm/messages?peer=' + encodeURIComponent(peer), { cache: 'no-store' });
...
async function renderChat(room = 'general', dmPeer = '') {
  ...
  try {
    const [roomsRes, dmRes, msgsRes] = await Promise.all([
      apiFetch('/api/chat/rooms', { cache: 'no-store' }),
      apiFetch('/api/chat/dms', { cache: 'no-store' }),
      apiFetch(currentChatMode === 'dm'
        ? '/api/chat/dm/messages?peer=' + encodeURIComponent(currentChatPeer)
...
function driveInlineArg(s) {
  return encodeURIComponent(String(s || ''));
}

function openDrivePath(path, replaceRoute = false) {
  navigate('drive', {path: normalizeDrivePathClient(path)}, replaceRoute);
}

function driveFileExt(name) {
  const clean = String(name || '').trim();
  const lastDot = clean.lastIndexOf('.');
  if (lastDot <= 0 || lastDot === clean.length - 1) return '';
  return clean.slice(lastDot + 1).toUpperCase();
}

function driveItemKind(it) {
  if (it.type === 'folder') return 'Folder';
  const ext = driveFileExt(it.name);
  return ext ? `${ext} file` : 'File';
}

function driveItemGlyph(it) {
  if (it.type === 'folder') return 'DIR';
  const ext = driveFileExt(it.name);
  return ext ? ext.slice(0, 3) : 'FILE';
}

function driveDisplaySize(it) {
  if (it.type === 'folder') return '--';
  if (it.size_bytes !== undefined && it.size_bytes !== null) return formatBytes(it.size_bytes);
  const raw = it.size;
  if (raw === undefined || raw === null || raw === '') return '0B';
  return /^\d+$/.test(String(raw)) ? formatBytes(raw) : String(raw);
}
...
async function renderDrive(folderPath = '/', _retry = 0) {
  ...
  try {
    [r, quota] = await Promise.all([
      apiFetch('/api/drive?path=' + encodeURIComponent(folderPath)).then(async x => {
        let data = null;
        try { data = await x.json(); } catch (_) {}
        if (!data || typeof data !== 'object') data = {ok: false, error: 'bad response'};
        if (x.status === 401) data.authenticated = false;
        return data;
      }),
      loadQuota(true)
    ]);
    ...
    if (!(await ensureBackendStorageAvailable())) return;
    content.innerHTML = '<div class="email-view">Error loading Drive.</div>';
    return;
  }
  ...
  const breadcrumbHtml = crumbs.map((c, i) => {
    ...
    return `${sep}<button class="crumb" type="button" onclick="openDrivePath(decodeURIComponent('${driveInlineArg(c.path)}'))">${escHtml(c.label)}</button>`;
  }).join('');

  const usedPct = quota ? Math.min(100, Math.round((quota.used_bytes / Math.max(1, quota.limit_bytes)) * 100)) : 0;
  const quotaLabel = quota
    ? `${formatQuotaUsedBytes(quota.used_bytes)} of ${formatBytes(quota.limit_bytes)} used`
    : 'Storage unavailable';
  const remainingLabel = quota ? `${formatBytes(quota.remaining_bytes)} free` : '';
  const emptyState = `
    <div class="drive-empty">
      <div class="drive-empty-icon">DIR</div>
      <div>
      ...
      ${items.length === 0
        ...
        : `<div class="drive-table-header">
             ...
             const openAction = it.type === 'folder'
               ? `openDrivePath(decodeURIComponent('${pathArg}'))`
          ...
          <div class="drive-row">
            <button class="drive-name-btn" type="button" onclick="${openAction}">
              <span class="drive-icon ${it.type === 'folder' ? 'folder' : 'file'}">${escHtml(driveItemGlyph(it))}</span>
            ...
            <div class="drive-row-actions">
              ...
              <button onclick="moveItem(decodeURIComponent('${pathArg}'))">Move</button>
...
function setUploadProgress({name = '', loaded = 0, total = 0, rate = 0, status = '', state = ''}) {
  ...
  document.getElementById('upload-progress-status').textContent =
    status || `${formatBytes(loaded)} of ${formatBytes(total)}`;
  document.getElementById('upload-progress-rate').textContent = rate > 0 ? `${formatBytes(rate)}/s` : '';
}

function hideUploadProgress(delay = 1800) {
  if (uploadProgressHideTimer) clearTimeout(uploadProgressHideTimer);
  uploadProgressHideTimer = setTimeout(() => {
    const box = document.getElementById('upload-progress');
    if (box) box.classList.remove('show', 'done', 'error');
    uploadProgressHideTimer = null;
  }, delay);
}

function uploadDriveForm(fd, file) {
  return new Promise((resolve, reject) => {
    ...
    xhr.upload.addEventListener('progress', ev => {
      if (!ev.lengthComputable) {
        setUploadProgress({name: file.name, status: 'Uploading...', rate: 0});
        return;
      }
      const now = performance.now();
      const elapsed = Math.max(1, now - lastAt);
      const instantRate = ((ev.loaded - lastLoaded) * 1000) / elapsed;
      const avgRate = (ev.loaded * 1000) / Math.max(1, now - started);
      const rate = instantRate > 0 ? (instantRate * 0.35 + avgRate * 0.65) : avgRate;
      lastLoaded = ev.loaded;
      lastAt = now;
      const status = ev.loaded >= ev.total
        ? 'Upload sent. Finalizing...'
        : `${formatBytes(ev.loaded)} of ${formatBytes(ev.total)}`;
      setUploadProgress({name: file.name, loaded: ev.loaded, total: ev.total, rate, status});
    });
    ...
    xhr.addEventListener('error', () => reject(new Error('storage connection lost')));
    xhr.addEventListener('abort', () => reject(new Error('upload cancelled')));

    xhr.open('POST', '/api/upload');
    const _usid = sessionStorage.getItem(_sidKey) || '';
    if (_usid) xhr.setRequestHeader('X-Session-Id', _usid);
    xhr.send(fd);
  });
}
...
async function uploadFile(input) {
  ...
  if (file.size > MAX_FILE_BYTES) {
    showToast(`Upload blocked: file exceeds 128 MB server limit (${formatBytes(file.size)})`);
    input.value = '';
    return;
  }
  ...
  setUploadProgress({name: file.name, loaded: 0, total: file.size, status: 'Starting...', rate: 0});
  try {
    const result = await uploadDriveForm(fd, file);
    const data = result.data;
    if (!result.ok) {
      const msg = (data && data.error) ? data.error : `HTTP ${result.status}`;
      setUploadProgress({name: file.name, loaded: 0, total: file.size, status: 'Upload failed: ' + msg, state: 'error'});
      showToast('Upload failed: ' + msg);
      hideUploadProgress(3500);
      return;
    }
    if (data && data.ok) {
      setUploadProgress({name: file.name, loaded: file.size, total: file.size, status: `Uploaded ${formatBytes(file.size)}`, state: 'done'});
      showToast('Uploaded!');
      quotaCache = null;
    ...
    const msg = e && e.message ? e.message : 'storage connection lost';
    setUploadProgress({name: file.name, loaded: 0, total: file.size, status: 'Upload failed: ' + msg, state: 'error'});
    showToast('Upload failed: ' + msg);
    hideUploadProgress(3500);
  } finally {
...
async function downloadFile(uid, name) {
  ...
  const _dsid = sessionStorage.getItem(_sidKey) || '';
  a.href = `/api/download/${encodeURIComponent(uid)}` + (_dsid ? '?sid=' + encodeURIComponent(_dsid) : '');
  a.download = name;
  a.click();
}
...
async function deleteItem(path) {
  ...
  const r = await apiFetch('/api/delete-path', {
    method: 'POST',
    headers: {'Content-Type': 'application/x-www-form-urlencoded'},
  ...
  content.innerHTML = `
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(320px,1fr));gap:20px;align-items:start">
      ...
      <div class="compose-form" style="max-width:none">
        ...
        <div style="font-size:13px;color:var(--muted);margin-bottom:12px">Per-user Drive quota with a 128MB maximum per uploaded file.</div>
        <input id="quota-mb" type="number" min="1" max="5120" value="${quota ? Math.round(quota.limit_bytes / (1024 * 1024)) : 5120}">
        <div style="font-size:13px;color:var(--muted);margin-bottom:12px">Currently using ${quota ? `${formatQuotaUsedBytes(quota.used_bytes)} of ${formatBytes(quota.limit_bytes)}` : 'unknown'}.</div>
...
async function deleteContact(email) {
  const r = await apiFetch('/api/contacts/delete', {
    method: 'POST',
    headers: {'Content-Type': 'application/x-www-form-urlencoded'},
...
function startSSE() {
  ...
  const _ssid = sessionStorage.getItem(_sidKey) || '';
  eventSource = new EventSource('/events' + (_ssid ? '?sid=' + encodeURIComponent(_ssid) : ''));
...
// Restore session ID passed via URL after a cross-origin frontend failover redirect
(function() {
  try {
    const _p = new URLSearchParams(window.location.search || '');
    const _s = _p.get('_sid');
    if (_s && !sessionStorage.getItem(_sidKey)) {
      sessionStorage.setItem(_sidKey, _s);
      _p.delete('_sid');
...
bootstrapInitialRoute();
    ...
    HttpResponse resp = HttpResponse::ok(html, "text/html; charset=utf-8");
    if (html.size() >= 64 * 1024) resp.chunked = true;
    return resp;
}
...
HttpResponse FEServer::handle_login(const HttpRequest& req) {
    ...
    HttpResponse resp = HttpResponse::json("{\"ok\":true,\"sid\":" + json_str(sid) + "}");
    resp.set_cookie("sid", sid);
    return resp;
}

HttpResponse FEServer::handle_logout(const HttpRequest& req) {
    std::string sid = sid_from_req(req);
    if (!sid.empty()) {
        sessions_->destroy_async(sid);
    }
...
HttpResponse FEServer::handle_session(const HttpRequest& req) {
    std::string user;
    SessionKVReadStatus st = sessions_->validate_status(sid_from_req(req), user);
...
HttpResponse FEServer::handle_change_password(const HttpRequest& req) {
    std::string user = sessions_->validate(sid_from_req(req));
    if (user.empty()) return HttpResponse::error(401, "Unauthorized");
...
HttpResponse FEServer::handle_admin_status(const HttpRequest&) {
    ...
    if (cfg_.coord_port > 0 && !cfg_.coord_host.empty()) {
        ...
        if (cfg_.kv_port > 0) {
            AdminClusterSpec configured = cfg_derived_cluster_spec_admin(cfg_);
            for (const auto& kv : configured.backends) {
                if (tcp_probe_admin("127.0.0.1", kv.second.kv_port, 80)) {
                    backend_json = fallback_backend_json_admin(cfg_);
                    tablets_json = "[]";
                    ok = true;
                    break;
                }
...
HttpResponse FEServer::handle_admin_control_redirect(const HttpRequest& req) {
    ...
    if (kind == "frontend" && action == "kill" && target_port == cfg_.port) {
        ...
        // to that peer's /admin page. The killed port itself gets a 404 stub.
        for (const auto& p : frontend_peer_order_admin()) {
            ...
            if (!frontend_admin_status_ok_admin(p.second)) continue; // skip stubs/dead
            {
                ...
                std::string kill_and_stub =
                    frontend_self_kill_helper_cmd_admin({my_port}, my_port, -1, fe_bin_loc);
                run_shell_command(kill_and_stub);
            }
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; }
    ...
    body {
      ...
      background: linear-gradient(180deg, #EAF2FB 0, var(--bg) 260px);
      color: var(--text);
      margin: 0;
      min-height: 100vh;
    }
    ...
    .admin-hero {
      ...
      box-shadow: 0 18px 42px rgba(1,31,91,0.22);
      margin-bottom: 18px;
    }
    .logo-lockup { display: flex; align-items: center; gap: 14px; min-width: 0; }
    .penn-logo { width: 42px; height: 49px; flex: 0 0 auto; }
    .eyebrow {
    ...
    @media (max-width: 820px) {
      .admin-shell { width: min(100vw - 24px, 1500px); padding-top: 12px; }
    ...
    @media (max-width: 520px) {
      ...
      h1 { font-size: 24px; }
      .penn-logo { width: 36px; height: 42px; }
    }
  </style>
</head>
<body>
  ...
  <div class="card">
    <table id="frontend-table">
	  ...
	  <div class="section-title">
	    ...
	    <div class="section-note">Replica assignment by inclusive tablet range</div>
	  </div>
	  <div class="card">
...
function renderFrontend(nodes) {
  ...
  for (const n of nodes) {
    ...
    tr.innerHTML = `
      ...
      <td>${aliveCell(!!n.alive)}</td>
      <td>
        <button onclick="adminControl('frontend','kill','${esc(n.id)}')">Kill</button>
...
function tabletRangeStart(t) {
  return t.row_start || 'min';
}

function tabletRangeEnd(t) {
  const end = t.row_end || '';
  if (!end) return 'z';
  if (/^[a-z]$/.test(end)) {
    const code = end.charCodeAt(0);
    return code > 97 ? String.fromCharCode(code - 1) : end;
  }
  return '< ' + end;
}
...
function renderTablets(tablets) {
  ...
  for (const t of tablets) {
    const reps = t.replicas || [];
    const rangeStart = tabletRangeStart(t);
    const rangeEnd = tabletRangeEnd(t);
    if (reps.length === 0) {
      const tr = document.createElement('tr');
      tr.innerHTML = `
        <td>${esc(t.name)}</td>
        <td>${esc(rangeStart)}</td>
        <td>${esc(rangeEnd)}</td>
    ...
    reps.forEach((r, idx) => {
      ...
      tr.innerHTML = `
        ...
        <td>${idx === 0 ? esc(rangeStart) : ''}</td>
        <td>${idx === 0 ? esc(rangeEnd) : ''}</td>
        <td>${esc(r.id)}</td>
        <td>${esc(r.role)}</td>
	...
	async function refreshRawData(resetOffset) {
	  ...
	  try {
	    ...
	    const r = await apiFetch(`/api/admin/raw?${params.toString()}`, {
	      cache: 'no-store',
	...
	function scheduleAdminRefreshBurst(kind, action) {
	  const delays = action === 'restart'
	    ? [80, 250, 600, 1000, 1600, 2500, 4000]
	    : [80, 250, 600, 1200, 2000];
	  for (const delay of delays) setTimeout(refreshStatus, delay);
	}
    ...
    return;
	  ...
	  try {
	    ...
	    const r = await apiFetch('/api/admin/control', {
	      method: 'POST',
	      headers,
    ...
    if (kind === 'frontend' && action === 'restart' && isCurrentFrontendTarget(target)) {
      adminStatusMutedUntil = Date.now() + 800;
    }
    ...
    scheduleAdminRefreshBurst(kind, action);
  } catch (e) {
    if (kind === 'frontend' && action === 'kill') {
      adminLastFrontendKillAt = Date.now();
      adminStatusMutedUntil = Date.now() + 10000;
...
async function refreshStatus() {
  ...
  const status = document.getElementById('status');
	  ...
	  try {
	    ...
	    renderReplicaValidation(tablets);
	    renderRawNodeOptions(data.backend_nodes || []);
	    if (!adminRawLoaded && adminRawNode) refreshRawData(false);
...
HttpResponse FEServer::handle_static(const HttpRequest& req) {
    ...
    HttpResponse resp = HttpResponse::ok(content, mime);
    if (content.size() >= 64 * 1024) resp.chunked = true;
    return resp;
}
...
constexpr size_t kMaxDriveFileBytes = 128ull * kMiB;
// Keep individual KV values small enough that synchronous replication and WAL
// flushes stay responsive when every replica is online.
constexpr size_t kDriveChunkSize = 1ull * kMiB;
constexpr size_t kMaxDriveQuotaBytes = 5ull * 1024ull * kMiB;
...
constexpr size_t kDefaultDriveQuotaBytes = kMaxDriveQuotaBytes;
...
std::string drive_human_size(size_t bytes) {
    ...
    if (unit == 0) return std::to_string(bytes) + units[unit];

    char buf[32];
    double rounded_one = std::round(value * 10.0) / 10.0;
    if (std::fabs(rounded_one - std::round(rounded_one)) < 0.0001) {
        std::snprintf(buf, sizeof(buf), "%.0f%s", rounded_one, units[unit]);
    } else {
        std::snprintf(buf, sizeof(buf), "%.1f%s", rounded_one, units[unit]);
    }
    return buf;
}
...
static const char* kUsedBytesCol = "used_bytes";

// O(1) used-bytes lookup: reads the cached counter, falls back to full traversal on miss.
size_t user_drive_used_bytes_fast(KVClient* kv, const std::string& user) {
    std::string cached = kv->get_str(drive_user_row(user), kUsedBytesCol);
    if (!cached.empty()) {
        try { return static_cast<size_t>(std::stoull(cached)); } catch (...) {}
    }
    size_t actual = user_drive_used_bytes(kv, user);
    kv->put(drive_user_row(user), kUsedBytesCol, std::to_string(actual));
    return actual;
}

size_t user_drive_used_bytes_reconciled(KVClient* kv, const std::string& user) {
    size_t actual = user_drive_used_bytes(kv, user);
    std::string actual_s = std::to_string(actual);
    std::string cached = kv->get_str(drive_user_row(user), kUsedBytesCol);
    if (cached != actual_s) {
        kv->put(drive_user_row(user), kUsedBytesCol, actual_s);
    }
    return actual;
}

// Atomically adjusts the cached counter by `delta` (negative for deletes).
void update_drive_used_bytes(KVClient* kv, const std::string& user, int64_t delta) {
    if (delta == 0) return;
    for (int attempt = 0; attempt < 20; ++attempt) {
        std::string old_s;
        KVReadStatus st = kv->get_status(drive_user_row(user), kUsedBytesCol, old_s);
        if (st == KVReadStatus::Unavailable) {
        ...
        size_t old_val = 0;
        if (st == KVReadStatus::Found) {
            try { old_val = static_cast<size_t>(std::stoull(old_s)); } catch (...) {}
        }
        int64_t nv = static_cast<int64_t>(old_val) + delta;
        std::string new_s = std::to_string(nv > 0 ? static_cast<size_t>(nv) : 0);
        if (st == KVReadStatus::Found) {
            if (old_s == new_s || kv->cput(drive_user_row(user), kUsedBytesCol, old_s, new_s)) return;
        } else {
            kv->put(drive_user_row(user), kUsedBytesCol, new_s);
            return;
        }
        std::this_thread::sleep_for(std::chrono::milliseconds(3));
    }
...
bool put_file_chunks(KVClient* kv, const std::string& uid, const std::string& data) {
    ...
    for (size_t i = 0; i < chunks; ++i) {
        ...
        bool ok = kv->put_bytes(row, drive_file_chunk_col(i), data.data() + off, len);
        if (!ok) {
...
bool ensure_drive_root(KVClient* kv, const std::string& user, std::string& root_uid) {
    root_uid.clear();
    KVReadStatus st = kv->get_status(drive_user_row(user), kDriveRootCol, root_uid);
    if (st == KVReadStatus::Found) {
        if (root_uid.empty()) return false;

        std::string type;
        KVReadStatus type_st = kv->get_status(drive_obj_row(root_uid), "type", type);
        if (type_st == KVReadStatus::Unavailable) return false;
        if (type != "folder") {
        ...
        std::string children;
        KVReadStatus child_st = kv->get_status(drive_dir_row(root_uid), "children", children);
        if (child_st == KVReadStatus::Unavailable) return false;
        if (child_st == KVReadStatus::NotFound) {
            if (!kv->put(drive_dir_row(root_uid), "children", "")) return false;
        }
        return true;
    }
    if (st == KVReadStatus::Unavailable) return false;
...
HttpResponse FEServer::handle_drive_list(const HttpRequest& req, const std::string& user) {
    ...
    {
        std::string probe;
        if (kv_->get_status(drive_user_row(user), kDriveRootCol, probe) == KVReadStatus::Unavailable)
            return HttpResponse::json(R"({"ok":false,"error":"storage temporarily unavailable, please try again"})");
    }
...
HttpResponse FEServer::handle_upload(const HttpRequest& req, const std::string& user) {
    ...
    for (auto& part : parts) {
        ...
        else if (part.name == "file") { file_part = std::move(part); have_file = true; }
    }
    ...
    if (file_part.data.size() > kMaxDriveFileBytes) {
        return HttpResponse::json(R"({"ok":false,"error":"file exceeds 128MB limit"})");
    }

    size_t used_bytes = user_drive_used_bytes_fast(kv_.get(), user);
    size_t limit_bytes = user_drive_quota_bytes(kv_.get(), user);
    if (used_bytes + file_part.data.size() > limit_bytes) {
        used_bytes = user_drive_used_bytes_reconciled(kv_.get(), user);
    }
    if (used_bytes + file_part.data.size() > limit_bytes) {
    ...
    bool wrote_type = false, wrote_name = false, wrote_parent = false,
         wrote_owner = false, wrote_size = false, wrote_chunks = false;

    if (!(wrote_type   = kv_->put(drive_obj_row(uid), "type",   "file")))
        fail_reason = "failed to store file type";
    else if (!(wrote_name  = kv_->put(drive_obj_row(uid), "name",   file_part.filename)))
        fail_reason = "failed to store file name";
    else if (!(wrote_parent = kv_->put(drive_obj_row(uid), "parent", parent_uid)))
        fail_reason = "failed to store parent folder";
    else if (!(wrote_owner  = kv_->put(drive_obj_row(uid), "owner",  user)))
        fail_reason = "failed to store file owner";
    else if (!(wrote_size   = kv_->put(drive_obj_row(uid), "size",
                                       std::to_string(file_part.data.size()))))
        fail_reason = "failed to store file size";
    else if (!(wrote_chunks = put_file_chunks(kv_.get(), uid, file_part.data)))
        fail_reason = "failed to store file data";
    else if (!append_child(kv_.get(), parent_uid, uid))
        fail_reason = "failed to update folder listing";

    if (!fail_reason.empty()) {
        // Only clean up cells that were actually written to avoid spurious
        // DEL commands on cells that were never created.
        if (wrote_type)   kv_->del(drive_obj_row(uid), "type");
        if (wrote_name)   kv_->del(drive_obj_row(uid), "name");
        if (wrote_parent) kv_->del(drive_obj_row(uid), "parent");
        if (wrote_owner)  kv_->del(drive_obj_row(uid), "owner");
        if (wrote_size)   kv_->del(drive_obj_row(uid), "size");
        if (wrote_chunks) delete_file_bytes(kv_.get(), uid);
        return HttpResponse::json("{\"ok\":false,\"error\":" + json_str(fail_reason) + "}");
    }

    update_drive_used_bytes(kv_.get(), user, static_cast<int64_t>(file_part.data.size()));
...
HttpResponse FEServer::handle_move(const HttpRequest& req, const std::string& user) {
    ...
    {
        std::string probe;
        KVReadStatus st = kv_->get_status(drive_user_row(user), kDriveRootCol, probe);
        if (st == KVReadStatus::Unavailable) {
            return HttpResponse::json(R"({"ok":false,"error":"storage temporarily unavailable, please try again"})");
        }
    }
    if (!resolve_path(kv_.get(), user, src_path, uid, &type)) {
        return HttpResponse::json(R"({"ok":false,"error":"source path not found"})");
    }
    if (!resolve_move_destination(kv_.get(), user, src_path, dst_path, resolved_dst_path, dst_uid, dst_type)) {
        return HttpResponse::json(R"({"ok":false,"error":"destination path not found"})");
    }
    std::string name = kv_->get_str(drive_obj_row(uid), "name");
...
HttpResponse FEServer::handle_mkdir(const HttpRequest& req, const std::string& user) {
    ...
    std::string uid = drive_new_uid();
    bool wr_type = kv_->put(drive_obj_row(uid), "type", "folder");
    bool wr_name = wr_type && kv_->put(drive_obj_row(uid), "name", name);
    bool wr_parent = wr_name && kv_->put(drive_obj_row(uid), "parent", parent_uid);
    bool wr_owner = wr_parent && kv_->put(drive_obj_row(uid), "owner", user);
    bool wr_children = wr_owner && kv_->put(drive_dir_row(uid), "children", "");
    bool wr_linked = wr_children && append_child(kv_.get(), parent_uid, uid);
    if (!wr_linked) {
        if (wr_type)     kv_->del(drive_obj_row(uid), "type");
        if (wr_name)     kv_->del(drive_obj_row(uid), "name");
        if (wr_parent)   kv_->del(drive_obj_row(uid), "parent");
        if (wr_owner)    kv_->del(drive_obj_row(uid), "owner");
        if (wr_children) kv_->del(drive_dir_row(uid), "children");
...
HttpResponse FEServer::handle_delete_path(const HttpRequest& req, const std::string& user) {
    ...
    for (auto it = nodes.rbegin(); it != nodes.rend(); ++it) {
        ...
        if (obj_type == "file") {
            std::string sz = kv_->get_str(drive_obj_row(obj), "size");
            freed_bytes += static_cast<int64_t>(parse_size_t_or(sz, 0));
        }
    ...
    if (freed_bytes > 0) update_drive_used_bytes(kv_.get(), user, -freed_bytes);
    return HttpResponse::json(R"({"ok":true})");
}
HttpResponse FEServer::handle_quota_status(const HttpRequest&, const std::string& user) {
    ...
    size_t limit_bytes = user_drive_quota_bytes(kv_.get(), user);
    size_t used_bytes = user_drive_used_bytes_reconciled(kv_.get(), user);
...
HttpResponse FEServer::handle_quota_update(const HttpRequest& req, const std::string& user) {
    ...
    // Quota controls aggregate Drive usage; each individual upload is still
    // capped separately by kMaxDriveFileBytes.
    constexpr size_t kMaxQuotaMB = kMaxDriveQuotaBytes / (1024ull * 1024ull);
    if (limit_mb == 0 || limit_mb > kMaxQuotaMB) {
        return HttpResponse::json(R"({"ok":false,"error":"limit must be between 1 and 5120 MB"})");
    }
