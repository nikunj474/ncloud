...
function routeFromLocation() {
  ...
  if (path === '/drive') return {view: 'drive', params: {path: '/'}};
...
async function fetchSessionUser() {
  ...
  const timeout = setTimeout(() => controller.abort(), 700);
  try {
    const r = await fetch('/api/session', { cache: 'no-store', signal: controller.signal });
...
function navigate(view, params = {}, replaceRoute = false) {
  ...
  if (replaceRoute) history.replaceState({view, params}, '', '/' + view);
  else history.pushState({view, params}, '', '/' + view);
...
async function uploadFile(input) {
  ...
  try {
    ...
    if (!r.ok) {
      showToast((data && data.error) ? 'Upload failed: ' + data.error : `Upload failed: HTTP ${r.status}`);
      return;
    }
    showToast(data && data.ok ? 'Uploaded!' : 'Upload failed: ' + ((data && data.error) || 'unknown error'));
    if (data && data.ok) {
      quotaCache = null;
      renderDrive(currentPath);
    }
  } catch (e) {
    showToast('Upload failed: storage connection lost');
  } finally {
    input.value = '';
  }
}
...
async function downloadFile(uid, name) {
  ...
  a.href = `/api/download/${encodeURIComponent(uid)}`;
  a.download = name;
  a.click();
}
  ...
  content.innerHTML = `
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(320px,1fr));gap:20px;align-items:start">
      ...
      <div class="compose-form" style="max-width:none">
        ...
        <input id="quota-mb" type="number" min="1" max="128" value="${quota ? Math.round(quota.limit_bytes / (1024 * 1024)) : 50}">
...
constexpr size_t kMaxDriveFileBytes = 128ull * 1024ull * 1024ull;
constexpr size_t kDriveChunkSize = kMaxDriveFileBytes;  // max supported file fits in one KV round trip
...
constexpr size_t kDefaultDriveQuotaBytes = kMaxDriveFileBytes;
...
bool put_file_chunks(KVClient* kv, const std::string& uid, const std::string& data) {
    ...
    for (size_t i = 0; i < chunks; ++i) {
        ...
        bool ok = false;
        if (off == 0 && len == data.size()) {
            ok = kv->put(row, drive_file_chunk_col(i), data);
        } else {
            ok = kv->put(row, drive_file_chunk_col(i), data.substr(off, len));
        }
        if (!ok) {
...
HttpResponse FEServer::handle_quota_update(const HttpRequest& req, const std::string& user) {
    ...
    constexpr size_t kMaxQuotaMB = 128;
    if (limit_mb == 0 || limit_mb > kMaxQuotaMB) {
        return HttpResponse::json(R"({"ok":false,"error":"limit must be between 1 and 128 MB"})");
    }
